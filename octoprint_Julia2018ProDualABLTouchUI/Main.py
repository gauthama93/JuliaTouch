...
class MainUiClass(QtWidgets.QMainWindow, mainGUI_pro_dual_abl.Ui_MainWindow):
    '''
    ...
    def __init__(self):
        ...
        file_handler = logging.FileHandler(r"C:\Users\Admin\Desktop\Julia2018ProDualABLTouchUI-master\ui.log")#("/home/pi/ui.log")
    ...
    def getFirmwareVersion(self):
        try:
            ...
            if req.status_code == requests.codes.ok:
                info = '\u2713' if not data["update_available"] else "\u2717"    # icon
                info += " Firmware: "
    ...
    def displayVersionInfo(self):
        ...
        if data:
            for item in data["information"]:
                ...
                info = '\u2713' if not plugin["updateAvailable"] else "\u2717"    # icon
    ...
    def acceptWifiSettings(self):
        ...
        wlan0_config_file.write("network={\n")
        wlan0_config_file.write('ssid="' + str(ascii_ssid) + '"\n')
        if self.hiddenCheckBox.isChecked():
            wlan0_config_file.write('scan_ssid=1\n')
        if str(self.wifiPasswordLineEdit.text()) != "":
            wlan0_config_file.write('psk="' + str(self.wifiPasswordLineEdit.text()) + '"\n')
        wlan0_config_file.write('}')
    ...
    def scan_wifi(self):
        ...
        scan_result = [s.strip() for s in scan_result]
        scan_result = [s.strip('"') for s in scan_result]
        scan_result = [_f for _f in scan_result if _f]
        return scan_result
    ...
    def changeFilament(self):
        ...
        self.changeFilamentComboBox.addItems(list(filaments.keys()))
